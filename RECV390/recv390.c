...
char			zoptbinary		= '-';		// eho 20071205: binary
char			zoptmember		= '-';		// eho 20071202: single member
...
unsigned char	line[LINE_SIZE];				// eho 20071130. EBCDIC data from TRANSMIT segment
...
int cmdline(int argc, char *argv[]) {
	...
	for (argnum = 1; ((opthit) && (argnum < argc)); argnum++) {
		...
		if (optchar) {
			arghit += parseopt(arg, "binary",			&zoptbinary); 	// eho 20071205
			arghit += parseopt(arg, "member",			&zoptmember); 	// eho 20071202
